...
public class ModernUIErosionRenderer {
    ...
    public static void registerCalls() {
        ModernTextRendererCall.registerCall(ModChatFormatting.EROSION, new Erosion());
    }

    public static class Erosion implements IModernTextRendererCall {
        ...
        @Override
        public void drawText(Object mEngine, FormattedCharSequence text, float x, float y, int color,
        ...
        @Override
        public float drawTextInstead(Object modernTextRenderer,
                Object mEngine,
                FormattedCharSequence text,
                float x, float y, int color,
                boolean dropShadow, Matrix4f matrix,
                MultiBufferSource source,
                Font.DisplayMode displayMode,
                int colorBackground, int packedLight,
                ModernTextRendererCall call) {
            try {

                String[] erosionParts = FontTextBuilder.formattedCharSequenceToStringErosion(text);

                Matrix4f originalMatrix = new Matrix4f(matrix);

                float erosionWidth = renderErosion(modernTextRenderer, erosionParts[0], x, y, color,
                        dropShadow, matrix, source, displayMode, colorBackground, packedLight);

                matrix.set(originalMatrix);
                java.lang.reflect.Method drawTextMethod = modernTextRenderer.getClass().getMethod(
                        "drawText", String.class, float.class, float.class, int.class, boolean.class,
                        Matrix4f.class, MultiBufferSource.class, Font.DisplayMode.class, int.class, int.class);
                float normalWidth = (Float) drawTextMethod.invoke(modernTextRenderer, erosionParts[1],
                        x, y, color, dropShadow, matrix, source, displayMode, colorBackground, packedLight);

                float result = Math.max(erosionWidth, normalWidth);
                return result;
            } catch (Exception e) {
                e.printStackTrace();
                return x;
        ...
        private float renderErosion(Object modernTextRenderer,
                String text, float startX, float startY,
                int iColor, boolean dropShadow,
            ...
            try {
                ...
                for (int i = 0; i < text.length(); i++) {
                    ...
                    posX = (Float) drawTextMethod.invoke(modernTextRenderer, String.valueOf(currentChar),
                            posX, startY, c, dropShadow, renderMatrix, bufferSource, displayMode, overlay, light);
                }

                return posX;
            } catch (Exception e) {
                GoetyAwaken.LOGGER.error("Error in renderErosion", e);
                e.printStackTrace();
                return startX;
            }
        }
