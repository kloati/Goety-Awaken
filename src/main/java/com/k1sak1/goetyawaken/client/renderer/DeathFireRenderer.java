...
@OnlyIn(Dist.CLIENT)
public class DeathFireRenderer extends EntityRenderer<DeathFire> {
    private final IceBouquetModel<DeathFire> model;

    public DeathFireRenderer(EntityRendererProvider.Context renderManagerIn) {
        super(renderManagerIn);
        this.model = new IceBouquetModel<>(renderManagerIn.bakeLayer(ModModelLayer.ICE_BOUQUET));
    }

    public void render(DeathFire pEntity, float entityYaw, float pPartialTicks, PoseStack pMatrixStack,
        ...
        if (f != 0.0F) {
            ...
            this.model.renderToBuffer(pMatrixStack, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, 1.0F,
                    1.0F, 1.0F, 0.15F);
    ...
    protected int getBlockLightLevel(DeathFire pEntity, BlockPos pPos) {
        return 15;
    }

    public ResourceLocation getTextureLocation(DeathFire entity) {
        return GoetyAwaken
                .location("textures/entity/projectiles/death_fire/" + entity.level().getGameTime() % 31 + ".png");
    }
}