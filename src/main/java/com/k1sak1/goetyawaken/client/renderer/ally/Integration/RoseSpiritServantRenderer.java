...
public class RoseSpiritServantRenderer extends MobRenderer<RoseSpiritServant, RoseSpiritServantModel> {
	...
	public RoseSpiritServantRenderer(Context context) {
		super(context, new RoseSpiritServantModel(context.bakeLayer(RoseSpiritServantModel.MODEL)), 0.5F);
		addLayer(new RoseSpiritServantGlowLayer(this));
	}

	@Override
	public void render(RoseSpiritServant entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn,
	...
	public static void renderCrystalBeams(float x, float y, float z, float partialTicks, int ticks,
		...
		for (int j = 1; j <= 8; ++j) {
			...
			float f9 = j / 8F;
			vertexconsumer.vertex(matrix4f, f4 * startScale, f5 * startScale, 0.0F).color(205, 112, 255, 255).uv(f6, f2)
					.overlayCoords(OverlayTexture.NO_OVERLAY).uv2(packedLightIn).normal(matrix3f, 0.0F, -1.0F, 0.0F)
					.endVertex();
			vertexconsumer.vertex(matrix4f, f4 * endScale, f5 * endScale, dist).color(0, 0, 0, 255).uv(f6, f3)
					.overlayCoords(OverlayTexture.NO_OVERLAY).uv2(packedLightIn).normal(matrix3f, 0.0F, -1.0F, 0.0F)
					.endVertex();
			vertexconsumer.vertex(matrix4f, f7 * endScale, f8 * endScale, dist).color(0, 0, 0, 255).uv(f9, f3)
					.overlayCoords(OverlayTexture.NO_OVERLAY).uv2(packedLightIn).normal(matrix3f, 0.0F, -1.0F, 0.0F)
					.endVertex();
			vertexconsumer.vertex(matrix4f, f7 * startScale, f8 * startScale, 0.0F).color(205, 112, 255, 255).uv(f9, f2)
					.overlayCoords(OverlayTexture.NO_OVERLAY).uv2(packedLightIn).normal(matrix3f, 0.0F, -1.0F, 0.0F)
					.endVertex();
			f4 = f7;
			f5 = f8;
	...
	@Override
	public ResourceLocation getTextureLocation(RoseSpiritServant entity) {
		return TEXTURE;
	}

	@Override
	public boolean shouldRender(RoseSpiritServant entity, Frustum frustrum, double p_114171_, double p_114172_,
			double p_114173_) {
