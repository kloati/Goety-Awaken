...
import com.k1sak1.goetyawaken.client.model.HeresiarchServantModel;
...
public class HeresiarchServantRenderer<T extends HeresiarchServant> extends MobRenderer<T, HeresiarchServantModel<T>> {
    protected static final ResourceLocation TEXTURE = GoetyAwaken.location("textures/entity/illager/heresiarch.png");
    ...
    public HeresiarchServantRenderer(EntityRendererProvider.Context p_174304_) {
        super(p_174304_, new HeresiarchServantModel<>(p_174304_.bakeLayer(ClientEventHandler.HERESIARCH_SERVANT_LAYER)),
                0.5F);
        this.addLayer(new ItemInHandLayer<>(this, p_174304_.getItemInHandRenderer()) {
            public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn,
                    ...
                    float headPitch) {
                if (heresiarchservant.getMainHandItem().is(ModTags.Items.WITCH_CURRENCY)) {
                    super.render(matrixStackIn, bufferIn, packedLightIn, heresiarchservant, limbSwing, limbSwingAmount,
                            partialTicks, ageInTicks, netHeadYaw, headPitch);
                }
            }
        });
        this.shadowModel = new HeresiarchServantModel<>(
                p_174304_.bakeLayer(ClientEventHandler.HERESIARCH_SERVANT_SHADOW_LAYER));
    }
        ...
        if (entity.isAlive()) {
            ...
            if (entity.trailSnapshots.isEmpty() || currentTick - entity.lastTrailTick > SNAPSHOT_INTERVAL) {
                if (entity.shouldAddTrailSnapshot()) {
                    Map<String, ModelPartPose> snapshot = ModelUtil.saveModelSnapshot(this.getModel().allPartNames,
                            this.getModel()::getAnyDescendantWithName);
                    entity.trailSnapshots.add(0, Pair.of(new Vec3(currentX, currentY, currentZ), new ModelSnapshot(0,
                            Mth.rotLerp(partialTicks, entity.yBodyRotO, entity.yBodyRot), currentTick, snapshot)));
            ...
            for (int i = 0; i < entity.trailSnapshots.size(); i++) {
                ...
                float modelAlpha = (1
                        ...
                        * 0.35F;
                if (modelAlpha > 0) {
                    this.shadowModel.renderToBuffer(poseStack, vertexConsumer, packedLight, OverlayTexture.NO_OVERLAY,
    ...
    private static void vertex(VertexConsumer p_254095_, Matrix4f p_254477_, Matrix3f p_253948_, int p_253829_,
            float p_253995_, int p_254031_, int p_253641_, int p_254243_) {
