...
public class VizierServantCapeLayer extends RenderLayer<VizierServant, VizierServantModel> {
    protected static final ResourceLocation DEFAULT_TEXTURE = GoetyAwaken
            .location("textures/entity/illager/vizier.png");

    public VizierServantCapeLayer(RenderLayerParent<VizierServant, VizierServantModel> p_117135_) {
        super(p_117135_);
    }

    public void render(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight, VizierServant pLivingEntity,
        ...
        if (!pLivingEntity.isInvisible()) {
            ...
            f1 = f1 + Mth.sin(Mth.lerp(pPartialTicks, pLivingEntity.walkDistO, pLivingEntity.walkDist) * 6.0F)
                    * 32.0F * f4;
            ...
            VertexConsumer ivertexbuilder = pBuffer
                    .getBuffer(RenderType.entityCutoutNoCull(this.getCapeTexture(pLivingEntity)));
            this.getParentModel().renderCape(pMatrixStack, ivertexbuilder, pPackedLight, OverlayTexture.NO_OVERLAY);
            pMatrixStack.popPose();
        }
    }

    public ResourceLocation getCapeTexture(VizierServant entity) {
