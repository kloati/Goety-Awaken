...
public abstract class AoeEntity extends Projectile {
    private static final EntityDataAccessor<Float> DATA_RADIUS = SynchedEntityData.defineId(AoeEntity.class,
            EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Boolean> DATA_CIRCULAR = SynchedEntityData.defineId(AoeEntity.class,
    ...
    protected boolean canHitEntity(Entity pTarget) {
        return (getOwner() != null && pTarget != getOwner() && !getOwner().isAlliedTo(pTarget))
        ...
        getParticle().ifPresent((particle) -> {
            ...
            for (int i = 0; i < f; i++) {
                ...
                if (isCircular()) {
                    ...
                    var theta = this.random.nextFloat() * 6.282f; // two pi
                    pos = new Vec3(
                ...
                Vec3 motion = new Vec3(
                        ...
                        (random.nextFloat() - 0.5f) * 0.03f).scale(this.getParticleSpeedModifier());

                level().addParticle(particle, getX() + pos.x, getY() + pos.y, getZ() + pos.z, motion.x, motion.y,
                        motion.z);
            }
        });
