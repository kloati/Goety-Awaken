...
public class GhostFireBolt extends SpellHurtingProjectile {
    private static final EntityDataAccessor<Integer> DATA_TYPE_ID = SynchedEntityData.defineId(GhostFireBolt.class,
            EntityDataSerializers.INT);
    public static final Map<Integer, ResourceLocation> TEXTURE_BY_TYPE = Util.make(Maps.newHashMap(), (map) -> {
        map.put(0, GoetyAwaken.location("textures/entity/projectiles/ghost_fire_bolt/ghost_fire_bolt_1.png"));
        map.put(1, GoetyAwaken.location("textures/entity/projectiles/ghost_fire_bolt/ghost_fire_bolt_2.png"));
        map.put(2, GoetyAwaken.location("textures/entity/projectiles/ghost_fire_bolt/ghost_fire_bolt_3.png"));
    });
    private final Vec3[] trailPositions = new Vec3[64];
    private int trailPointer = -1;

    public GhostFireBolt(EntityType<? extends AbstractHurtingProjectile> p_36833_, Level p_36834_) {
        super(p_36833_, p_36834_);
    }

    public GhostFireBolt(double pX, double pY, double pZ, double pXPower, double pYPower, double pZPower,
            Level pLevel) {
        super(ModEntityType.GHOST_FIRE_BOLT.get(), pX, pY, pZ, pXPower, pYPower, pZPower, pLevel);
    }

    public GhostFireBolt(LivingEntity pShooter, double pXPower, double pYPower, double pZPower, Level pLevel) {
        super(ModEntityType.GHOST_FIRE_BOLT.get(), pShooter, pXPower, pYPower, pZPower, pLevel);
    }
    ...
    protected void onHit(HitResult p_37628_) {
        ...
        if (!this.level().isClientSide) {
            ...
            Vec3 hitPosition = p_37628_.getLocation();
            IceBouquet ghostFire = new IceBouquet(
                    this.level(),
                    hitPosition.x(),
                    hitPosition.y(),
                    hitPosition.z(),
    ...
    public void hitDetection() {
        HitResult hitresult = net.minecraft.world.entity.projectile.ProjectileUtil.getHitResultOnMoveVector(this,
                this::canHitEntity);
    ...
    public void trailParticle() {
        ...
        if (this.isInWater()) {
            for (int i = 0; i < 4; ++i) {
                float f1 = 0.25F;
                this.level().addParticle(net.minecraft.core.particles.ParticleTypes.BUBBLE, d0 - vec3.x * f1,
                        d1 - vec3.y * f1, d2 - vec3.z * f1, vec3.x, vec3.y, vec3.z);
            }
        }
    ...
    protected boolean canHitEntity(Entity pEntity) {
        return pEntity != this.getOwner() && super.canHitEntity(pEntity);
    }
