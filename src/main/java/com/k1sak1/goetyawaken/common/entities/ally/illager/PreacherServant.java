...
public class PreacherServant extends SpellcasterIllagerServant {
    protected static final EntityDataAccessor<Byte> DATA_FLAGS_ID = SynchedEntityData.defineId(PreacherServant.class,
    ...
    public PreacherServant(EntityType<? extends PreacherServant> p_32627_, Level p_32628_) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new FollowMobClassGoal(this, 1.0F, 8.0F, 16.0F, (p_25278_) -> {
            return p_25278_ instanceof RaiderServant && p_25278_.getClass() != this.getClass()
                    && p_25278_.getHealth() < p_25278_.getMaxHealth()
                    && !(p_25278_ instanceof AllyIrk)
                    && !(p_25278_ instanceof AllyVex)
                    && !(p_25278_ instanceof TormentorServant)
                    && MobUtil.areAllies(this, p_25278_);
        }));
    }
    ...
    public class PreacherHealGoal extends Goal {
            ...
            TargetingConditions targetConditions = TargetingConditions.forNonCombat().range(this.getFollowDistance())
                    .selector(
                            p_26058_ -> p_26058_ instanceof LivingEntity
                                    ...
                                    && !(p_26058_ instanceof AllyVex)
                                    && !(p_26058_ instanceof TormentorServant)
                                    && MobUtil.areAllies(this.preacher, p_26058_)
                                    && this.preacher.hasLineOfSight(p_26058_));
            this.target = this.preacher.level()
                    .getNearestEntity(this.preacher.level().getEntitiesOfClass(LivingEntity.class,
                            this.getTargetSearchArea(this.getFollowDistance()), (p_148152_) -> {
                                return true;
                            }), targetConditions, this.preacher, this.preacher.getX(), this.preacher.getEyeY(),
                            this.preacher.getZ());
        }
        ...
        public void start() {
            ...
            this.preacher.playSound(this.preacher.getCastingSoundEvent(), this.preacher.getSoundVolume(),
                    this.preacher.getVoicePitch());
        }
                ...
                if (this.preacher.healTick == 10) {
                    ...
                    if (!this.preacher.level().isClientSide) {
                        ...
                        for (int i = 0; i < serverWorld.random.nextInt(10) + 10; ++i) {
                            serverWorld.sendParticles(ModParticleTypes.HEAL_EFFECT_2.get(),
                                    this.target.getRandomX(1.5D), this.target.getRandomY(),
                                    this.target.getRandomZ(1.5D), 0, 0.0F, 1.0F, 0.0F, 1.0F);
                        }
                        this.target.playSound(ModSounds.HEAL_SPELL.get(), this.preacher.getSoundVolume(),
                                this.preacher.getVoicePitch());
                    }
