...
public class CorruptedSoulBolt extends SpellHurtingProjectile {
    private static final EntityDataAccessor<Integer> DATA_TYPE_ID = SynchedEntityData.defineId(CorruptedSoulBolt.class,
            EntityDataSerializers.INT);
    public static final Map<Integer, ResourceLocation> TEXTURE_BY_TYPE = Util.make(Maps.newHashMap(), (map) -> {
        map.put(0, GoetyAwaken.location("textures/entity/projectiles/corrupted_soul_bolt/corrupted_soul_bolt_1.png"));
        map.put(1, GoetyAwaken.location("textures/entity/projectiles/corrupted_soul_bolt/corrupted_soul_bolt_2.png"));
        map.put(2, GoetyAwaken.location("textures/entity/projectiles/corrupted_soul_bolt/corrupted_soul_bolt_3.png"));
    });
    private final Vec3[] trailPositions = new Vec3[64];
    private int trailPointer = -1;

    public CorruptedSoulBolt(EntityType<? extends AbstractHurtingProjectile> p_36833_, Level p_36834_) {
        super(p_36833_, p_36834_);
    }

    public CorruptedSoulBolt(double pX, double pY, double pZ, double pXPower, double pYPower, double pZPower,
            Level pLevel) {
        super(ModEntityType.CORRUPTED_SOUL_BOLT.get(), pX, pY, pZ, pXPower, pYPower, pZPower, pLevel);
    }

    public CorruptedSoulBolt(LivingEntity pShooter, double pXPower, double pYPower, double pZPower, Level pLevel) {
        super(ModEntityType.CORRUPTED_SOUL_BOLT.get(), pShooter, pXPower, pYPower, pZPower, pLevel);
    }
    ...
    protected void onHitEntity(EntityHitResult p_37626_) {
        ...
        if (!this.level().isClientSide) {
            ...
            if (flag && entity instanceof LivingEntity living) {
                MobEffect mobEffect = MobEffects.WEAKNESS;
    ...
    protected void onHit(HitResult p_37628_) {
        ...
        if (!this.level().isClientSide) {
            ...
            for (int p = 0; p < 32; ++p) {
                ...
                double[] colors = MathHelper.rgbParticle(0xFF00FF);
