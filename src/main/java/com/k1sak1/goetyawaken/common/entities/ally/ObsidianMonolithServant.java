...
public class ObsidianMonolithServant extends AbstractMonolith implements IServant {
    protected static final EntityDataAccessor<Float> DATA_PROGRESS = SynchedEntityData.defineId(
            ObsidianMonolithServant.class,
            EntityDataSerializers.FLOAT);
    protected static final EntityDataAccessor<Integer> DATA_LEVEL = SynchedEntityData.defineId(
            ObsidianMonolithServant.class,
            EntityDataSerializers.INT);
    protected static final EntityDataAccessor<Integer> DATA_LEVELING = SynchedEntityData
            .defineId(ObsidianMonolithServant.class, EntityDataSerializers.INT);
    public int startUp;
    ...
    public ObsidianMonolithServant(EntityType<? extends AbstractMonolith> type, Level worldIn) {
        super(type, worldIn);
    }
    ...
    public void setConfigurableAttributes() {
        ...
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR),
                AttributesConfig.ObsidianMonolithArmor.get());
    }
    ...
    public void addAdditionalSaveData(CompoundTag p_31485_) {
        ...
        p_31485_.putBoolean("upgraded", this.upgraded);
    }
    ...
    public void readAdditionalSaveData(CompoundTag p_31474_) {
        ...
        if (p_31474_.contains("upgraded")) {
            this.upgraded = p_31474_.getBoolean("upgraded");
        }
    }
    ...
    public static boolean checkOMSpawnRules(EntityType<? extends Owned> p_217058_, LevelAccessor p_217059_,
        ...
        List<ObsidianMonolithServant> monoliths = p_217059_.getEntitiesOfClass(ObsidianMonolithServant.class,
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        ...
        if (!this.level().isClientSide && !this.isEmerging()) {
            ...
            if (pSource.is(DamageTypes.IN_WALL) || pSource.is(DamageTypes.OUTSIDE_BORDER)) {
                if (this.getTrueOwner() instanceof ApostleServant apostle) {
    ...
    public void silentDie(DamageSource cause) {
        ...
        if (this.level() instanceof ServerLevel serverLevel) {
            ...
            new SpellExplosion(this.level(), null, this.damageSources().magic(), this.blockPosition(), 16, 5.0F) {
                ...
                public void explodeHurt(Entity target, DamageSource damageSource, double x, double y, double z,
                    ...
                    if (target instanceof IOwned owned && target instanceof LivingEntity living) {
                        if (owned.getTrueOwner() == ObsidianMonolithServant.this.getTrueOwner()
                                || owned.getTrueOwner() == ObsidianMonolithServant.this) {
                            if (!(owned instanceof ObsidianMonolithServant)) {
                                if (living.hurt(living.damageSources().magic(),
                                        ObsidianMonolithServant.this.level().random.nextInt(10) + 5.0F)) {
                                    ObsidianMonolithServant.this.launch(living, ObsidianMonolithServant.this);
                                }
                            }
                        }
                    }
            ...
            if (this.getTrueOwner() instanceof ApostleServant apostle && apostle.isAlive()) {
    ...
    public boolean isCurrentlyGlowing() {
        return Config.obsidianMonolithGlow
                && (!this.level().isClientSide() && !this.isEmerging() && this.getTrueOwner() != null)
        ...
        if (!this.isEmerging()) {
            ...
            if (!this.isActivate()) {
                ...
                if (this.tickCount % 50 == 0) {
                    ...
                    for (CultistServant cultistServant : this.level().getEntitiesOfClass(CultistServant.class,
                            this.getBoundingBox().inflate(16.0D))) {
                        if (!(cultistServant instanceof com.k1sak1.goetyawaken.common.entities.ally.illager.ApostleServant)
                                && MobUtil.areAllies(this, cultistServant)
                                && cultistServant.getHealth() < cultistServant.getMaxHealth()) {
                            cultistServant.heal(1.0F);
                        }
                    }
            ...
            if (this.getTrueOwner() instanceof ApostleServant apostle) {
                if (Config.obsidianMonolithGlow) {
                ...
                if (apostle.distanceToSqr(this) > 1024.0D) {
                    this.teleportTowards(apostle);
                }
                ...
                if (apostle.getTarget() != null) {
                    ...
                    if (this.tickCount % time == 0 && i < j && this.level().random.nextFloat() <= 0.25F
                        ...
                        if (!this.level().isClientSide) {
                            ...
                            if (apostle.isApostleUpgraded()) {
                                this.summonAdditionalServants(apostle, serverLevel);
                            }
                        }
                    }
                }
            ...
            // int i1 = this.blockPosition().getX() + (Mth.randomBetweenInclusive(random1,
            // 4, 12)
            // * Mth.randomBetweenInclusive(random1, -1, 1));
            // int j1 = this.blockPosition().getY() + (Mth.randomBetweenInclusive(random1,
            // 0, 3)
            // * Mth.randomBetweenInclusive(random1, -1, 1));
            // int k1 = this.blockPosition().getZ() + (Mth.randomBetweenInclusive(random1,
            // 4, 12)
            // * Mth.randomBetweenInclusive(random1, -1, 1));
            // BlockPos blockPos = BlockFinder.SummonPosition(maverick, new BlockPos(i1, j1,
            // k1));
            // maverick.setPos(blockPos.getX() + 0.5D, blockPos.getY(), blockPos.getZ() +
            // 0.5D);
            ...
            // 4, 12)
            // * Mth.randomBetweenInclusive(random1, -1, 1));
            // BlockPos blockPos = BlockFinder.SummonPosition(heretic, new BlockPos(i1, j1,
            // k1));
            // heretic.setPos(blockPos.getX() + 0.5D, blockPos.getY(), blockPos.getZ() +
            // 0.5D);
            // if (this.level().noCollision(heretic.getBoundingBox())
            // && this.level().getEntityCollisions(heretic,
            // heretic.getBoundingBox()).isEmpty()
    ...
    private void spawnHeretics() {
        for (int i = 0; i < 4; ++i) {
            ...
            com.Polarice3.Goety.common.entities.ally.illager.cultist.HereticServant heretic = new HereticServant(
                    com.Polarice3.Goety.common.entities.ModEntityType.HERETIC_SERVANT.get(), this.level());
            BlockPos blockPos = BlockPos.containing(this.getX() + x, this.getY() - 1,
                    this.getZ() + z);
            if (!this.level().getBlockState(blockPos).getCollisionShape(this.level(),
                    blockPos).isEmpty()) {
    ...
    private void summonAdditionalServants(ApostleServant apostle, ServerLevel serverLevel) {
        RandomSource random = serverLevel.random;

        switch (apostle.getTitleNumber()) {
            case 2 -> {
                if (random.nextBoolean()) {
                    this.summonServant(serverLevel, apostle, "goety:mossy_skeleton_servant");
                }
                if (random.nextBoolean()) {
                    this.summonServant(serverLevel, apostle, "goety:jungle_zombie_servant");
                }
                if (random.nextBoolean()) {
                    this.summonServant(serverLevel, apostle, "goety:cave_spider_servant");
                }
                if (random.nextBoolean()) {
                    this.summonServant(serverLevel, apostle, "goety:muck_wraith_servant");
                }
            }
            case 3 -> {
                int count = random.nextInt(5);
                for (int i = 0; i < count; i++) {
                    this.summonServant(serverLevel, apostle, "goety:wither_skeleton_servant");
                }
            }
            case 4 -> {
                int count = random.nextInt(4);
                for (int i = 0; i < count; i++) {
                    this.summonServant(serverLevel, apostle, "goety:phantom_servant");
                }
            }
            case 6 -> {
                if (random.nextBoolean()) {
                    this.summonMagmaCubeServant(serverLevel, apostle);
                }
            }
            case 7 -> {
                int count = random.nextInt(3);
                for (int i = 0; i < count; i++) {
                    this.summonServant(serverLevel, apostle, "goety:husk_servant");
                }
            }
            case 8 -> {
                if (random.nextBoolean()) {
                    this.summonServant(serverLevel, apostle, "goety:stray_servant");
                }
                if (random.nextBoolean()) {
                    this.summonServant(serverLevel, apostle, "goety:frozen_zombie_servant");
                }
                if (random.nextBoolean()) {
                    this.summonServant(serverLevel, apostle, "goety:ice_golem");
                }
            }
            case 11 -> {
                if (random.nextBoolean()) {
                    this.summonServant(serverLevel, apostle, "goetyawaken:tormentor_servant");
                }
            }
            default -> {
            }
    ...
    private void summonServant(ServerLevel serverLevel, ApostleServant apostle, String entityType) {
        try {
            net.minecraft.resources.ResourceLocation resourceLocation = new net.minecraft.resources.ResourceLocation(
                    entityType);
            EntityType<?> entityTypeObj = net.minecraftforge.registries.ForgeRegistries.ENTITY_TYPES
                    .getValue(resourceLocation);
            if (entityTypeObj != null) {
                Entity entity = entityTypeObj.create(serverLevel);
                if (entity instanceof LivingEntity livingEntity) {
                    BlockPos.MutableBlockPos blockPos = this.blockPosition().mutable().move(
                            serverLevel.random.nextInt(5) - 2, 0, serverLevel.random.nextInt(5) - 2);
                    SummonCircle summonCircle = new SummonCircle(serverLevel, blockPos, livingEntity, true, true,
                            apostle);
    ...
    private void summonMagmaCubeServant(ServerLevel serverLevel, ApostleServant apostle) {
        try {
            net.minecraft.resources.ResourceLocation resourceLocation = new net.minecraft.resources.ResourceLocation(
            ...
            if (entityTypeObj != null) {
                Entity entity = entityTypeObj.create(serverLevel);
                if (entity instanceof LivingEntity livingEntity) {
                    if (entity instanceof com.Polarice3.Goety.common.entities.ally.MagmaCubeServant magmaCubeServant) {
                        magmaCubeServant.setSize(5, true);
                    }
                    BlockPos.MutableBlockPos blockPos = this.blockPosition().mutable().move(
                            serverLevel.random.nextInt(5) - 2, 0, serverLevel.random.nextInt(5) - 2);
                    if (livingEntity instanceof IOwned owned) {
                        owned.setTrueOwner(apostle);
                    }
                    SummonCircle summonCircle = new SummonCircle(serverLevel, blockPos, livingEntity, true, true,
                            apostle);
                    serverLevel.addFreshEntity(summonCircle);
    ...
    public void setCommandPosEntity(@Nullable LivingEntity living) {
        this.setCommandPos(this.blockPosition(), false);
