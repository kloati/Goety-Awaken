...
public class ColorUtil {
   ...
   public int colorCode(float alpha) {
      return ARGB32.color(Math.round(alpha * 255.0F), Math.round(this.red * 255.0F), Math.round(this.green * 255.0F),
            Math.round(this.blue * 255.0F));
   }
   ...
   public static class ARGB {
      ...
      public static int color(Vec3 vec3) {
         return color(as8BitChannel((float) vec3.x()), as8BitChannel((float) vec3.y()),
      ...
      public static int multiply(int color, int alpha) {
         if (color == -1) {
            ...
            return alpha == -1 ? color
                  : color(alpha(color) * alpha(alpha) / 255, red(color) * red(alpha) / 255,
                        green(color) * green(alpha) / 255, blue(color) * blue(alpha) / 255);
         }
      }
      ...
      public static int scaleRGB(int color, float redAlpha, float greenAlpha, float blueAlpha) {
         return color(alpha(color), Mth.clamp((int) ((float) red(color) * redAlpha), 0, 255),
      ...
      public static int scaleRGB(int color, int alpha) {
         return color(alpha(color), Mth.clamp(red(color) * alpha / 255, 0, 255),
               Mth.clamp(green(color) * alpha / 255, 0, 255), Mth.clamp(blue(color) * alpha / 255, 0, 255));
      }
      ...
      public static int lerp(float partialTick, int colorFrom, int colorTo) {
         return ARGB32.lerp(partialTick, colorFrom, colorTo);
      }

      public static int opaque(int color) {
         return ABGR32.opaque(color);
      }

      public static int transparent(int color) {
         return ABGR32.transparent(color);
      }
      ...
      public static int average(int color, int color2) {
         return color((alpha(color) + alpha(color2)) / 2, (red(color) + red(color2)) / 2,
               (green(color) + green(color2)) / 2, (blue(color) + blue(color2)) / 2);
      }
